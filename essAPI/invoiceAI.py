...
# Most important to build dataframes used as dataset or input to ml model
import pandas as pd
...
class InvoiceParser:
    def __init__(self, invoice_data, invoice_data_type):
        ...
        if invoice_data_type == 'pdf':
            ...
            for page in invoice.pages:
                ...
                img = np.array(page.to_image(
                    resolution=300).original.convert('L'))
            ...
            text = pytesseract.image_to_string(
                img, lang='eng', config=rf'--tessdata-dir "{absolute_path}"')
            self.extract(text)
            # dataframe contains positional features of each word present in invoice.
            df = pytesseract.image_to_data(
                img, lang='eng', config=rf'--tessdata-dir "{absolute_path}"', output_type='data.frame')
            if not df.empty:
    ...
    def extract_amount(self, data, height, width):
        ...
        prediction = model.predict(
            (field_id, cand_pos, neighbours, neighbour_positions))
